...
public class GoogleAnalyticsSettingsEditor : Editor {
	...
	GUIContent CampaignTrackingLabel = new GUIContent("Campaigns [?]:", "Auto Campaign Tracking. Avaliable on Android Only");
	...
	private void AutoTracking() {
		...
		if (GoogleAnalyticsSettings.Instance.showAdditionalParams) {
			...
			settings.AutoCampaignTracking = EditorGUILayout.Toggle(CampaignTrackingLabel, settings.AutoCampaignTracking);
